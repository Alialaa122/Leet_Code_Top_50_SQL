...
-- SELECT product_id,  ROUND(SUM(revenue)*1.0 / sum(DISTINCT units),2) AS average_price
-- FROM cte1 
-- GROUP BY product_id
--------------------------------------------------------------------------------------------------
...
SELECT product_id,     COALESCE(
        ROUND(SUM(revenue) * 1.0 / NULLIF(SUM(units), 0), 2),
        0
    ) AS average_price -- Removed Akward Distinct units in the first solution 
FROM cte1
GROUP BY product_id
